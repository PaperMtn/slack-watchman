...
- **[Medium]** Canvas URLs are now normalised before concatenation. The previous f-string `f'{workspace_information.url}canvas/{channel.id}'` produced a malformed URL like `https://example.slack.comcanvas/C123` whenever `team.info` returned a workspace URL without a trailing slash. A small `_build_canvas_url(workspace_url, channel_id)` helper now strips a trailing `/` from the base before concatenation, so both shapes resolve to a single `/` between host and path. Fixes [#126](https://github.com/PaperMtn/slack-watchman/issues/126)
- **[Medium]** The `after:` query timeframe is now formatted in UTC (`time.gmtime`) instead of the host's local timezone (`time.localtime`). Slack interprets `after:` in the workspace's timezone, so a host TZ that differed from the workspace TZ caused an off-by-one-day window at the boundary — common for CI runners, cloud hosts, or distributed teams. The four inline branches in `main()` have also been factored into a small `_compute_timeframe(tm, now)` helper so the TZ behaviour is unit-testable in isolation. Fixes [#125](https://github.com/PaperMtn/slack-watchman/issues/125)
...
- **[Low]** `SlackClient.cursor_api_search` now updates `cursor` once after the inner per-item loop instead of reassigning it on every iteration of the loop. On a normal page the previous code rewrote `cursor` to the same value N times (wasted work); on a page where `r.get(scope)` came back empty the inner loop never ran, so `cursor` was never updated and the outer `while` would re-request the same cursor — a potential infinite loop on empty pages with a non-empty `next_cursor`. The cursor is now read once per page after the items have been collected.
- **[Low]** Top-level `OUTPUT_LOGGER` annotation in `__init__.py` widened from `JSONLogger` to `StdoutLogger | JSONLogger` to match the actual runtime type returned by `init_logger`.
- **[Low]** Added explicit parentheses to `if everything or (not pii and not secrets):` in `main()`. Behaviour is unchanged (Python's precedence already evaluates `not` and `and` before `or`) but the grouping is no longer ambiguous to a reader skimming the branch.
